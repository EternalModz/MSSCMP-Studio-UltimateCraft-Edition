...
namespace MSSCMP_Studio.Classes.Bink
{
	internal class Bink
	{
		...
		public unsafe static string Binka(string infile, string outDir = null, bool last = true, string working = null)
		{
			...
			if (flag)
			{
				working = Path.GetTempPath() + DateTime.Now.Second;
				text = ExtractResource("MSSCMP_Studio.Resources.binka_encode.exe", working, "binka_encode.exe");
				text2 = ExtractResource("MSSCMP_Studio.Resources.mss32.dll", working, "mss32.dll");
				path = ExtractResource("MSSCMP_Studio.Resources.binkawin.asi", working, "binkawin.asi");
